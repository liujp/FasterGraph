...
enum class FlushStatus : uint8_t { Flushed, InProgress };

enum class CloseStatus : uint8_t { Closed, Open };
...
struct FlushCloseStatus {
  FlushCloseStatus() : flush{FlushStatus::Flushed}, close{CloseStatus::Closed} {}

  FlushCloseStatus(FlushStatus flush_, CloseStatus close_) : flush{flush_}, close{close_} {}

  FlushCloseStatus(uint16_t control_) : control{control_} {}
...
class AtomicFlushCloseStatus {
 public:
  AtomicFlushCloseStatus() : status_{} {}
...
struct FullPageStatus {
  FullPageStatus() : LastFlushedUntilAddress{0}, status{} {}
...
class PageOffset {
 public:
  PageOffset(uint32_t page, uint64_t offset) : offset_{offset}, page_{page} {
    assert(page <= Address::kMaxPage);
  }

  PageOffset(uint64_t control) : control_{control} {}

  PageOffset(const Address& address) : offset_{address.offset()}, page_{address.page()} {}
...
class AtomicPageOffset {
 public:
  AtomicPageOffset() : control_{0} {}

  AtomicPageOffset(uint32_t page, uint64_t offset) : control_{PageOffset{page, offset}.control()} {}
...
class PersistentMemoryMalloc {
 public:
  ...
  inline Address GetTailAddress() const {
    PageOffset tail_page_offset = tail_page_offset_.load();
    return Address{
        tail_page_offset.page(),
        std::min(Address::kMaxOffset, static_cast<uint32_t>(tail_page_offset.offset()))};
  }
 ...
 private:
  inline void GetFileReadBoundaries(
      Address read_offset,
  ...
  inline bool MonotonicUpdate(A& variable, T new_value, T& old_value) {
  ...
  Status
  AsyncFlushPages(uint32_t start_page, Address until_address, bool serialize_objects = false);

 public:
  Status AsyncFlushPagesToFile(
      uint32_t start_page,
  ...
  /// Recovery.
  Status
  AsyncReadPagesFromLog(uint32_t start_page, uint32_t num_pages, RecoveryStatus& recovery_status);
...
template <class D>
inline void PersistentMemoryMalloc<D>::AsyncGetFromDisk(
    Address address,
    uint32_t num_records,
    AsyncIOCallback callback,
    AsyncIOContext& context) {
...
void PersistentMemoryMalloc<D>::OnPagesClosed(IAsyncContext* ctxt) {
  ...
  if (context->allocator->MonotonicUpdate(
          context->allocator->safe_head_address,
    ...
    for (uint32_t idx = old_safe_head_address.page(); idx < context->new_safe_head_address.page();
      ...
      } while (!context->allocator->PageStatus(idx).status.compare_exchange_weak(
...
void PersistentMemoryMalloc<D>::OnPagesMarkedReadOnly(IAsyncContext* ctxt) {
  ...
  if (context->allocator->MonotonicUpdate(
          context->allocator->safe_read_only_address,
          context->new_safe_read_only_address,
          old_safe_read_only_address)) {
    context->allocator->AsyncFlushPages(
        old_safe_read_only_address.page(), context->new_safe_read_only_address);
  }
}

template <class D>
Status PersistentMemoryMalloc<D>::AsyncFlushPages(
    uint32_t start_page,
    Address until_address,
  ...
  auto callback = [](IAsyncContext* ctxt, Status result, size_t bytes_transferred) {
    ...
    context->allocator->PageStatus(context->page)
        .LastFlushedUntilAddress.store(context->until_address);
    ...
    } while (!context->allocator->PageStatus(context->page)
                  .status.compare_exchange_weak(old_status, new_status));
    if (old_status.close == CloseStatus::Closed) {
      ...
      context->allocator->PageStatus(context->page)
          .status.store(FlushStatus::Flushed, CloseStatus::Open);
    }
  ...
  for (uint32_t flush_page = start_page; flush_page < start_page + num_pages; ++flush_page) {
    ...
    RETURN_NOT_OK(
        file->WriteAsync(Page(flush_page), kPageSize * flush_page, kPageSize, callback, context));
  }
  return Status::Ok;
}

template <class D>
Status PersistentMemoryMalloc<D>::AsyncFlushPagesToFile(
    uint32_t start_page,
    Address until_address,
    file_t& file,
    std::atomic<uint32_t>& flush_pending) {
  class Context : public IAsyncContext {
   public:
    Context(std::atomic<uint32_t>& flush_pending_) : flush_pending{flush_pending_} {}
    /// The deep-copy constructor
    Context(Context& other) : flush_pending{other.flush_pending} {}
  ...
  for (uint32_t flush_page = start_page; flush_page < start_page + num_pages; ++flush_page) {
    ...
    RETURN_NOT_OK(file.WriteAsync(
        Page(flush_page), kPageSize * (flush_page - start_page), kPageSize, callback, context));
  }
  return Status::Ok;
}

template <class D>
Status PersistentMemoryMalloc<D>::AsyncReadPagesFromLog(
    uint32_t start_page,
    uint32_t num_pages,
    RecoveryStatus& recovery_status) {
...
template <class D>
Status PersistentMemoryMalloc<D>::AsyncReadPagesFromSnapshot(
    file_t& snapshot_file,
    uint32_t file_start_page,
    uint32_t start_page,
    uint32_t num_pages,
...
template <class D>
template <class F>
Status PersistentMemoryMalloc<D>::AsyncReadPages(
    F& read_file,
    uint32_t file_start_page,
    uint32_t start_page,
    uint32_t num_pages,
  ...
  class Context : public IAsyncContext {
   public:
    Context(std::atomic<PageRecoveryStatus>& page_status_) : page_status{&page_status_} {}
    /// The deep-copy constructor
    Context(const Context& other) : page_status{other.page_status} {}
  ...
  for (uint32_t read_page = start_page; read_page < start_page + num_pages; ++read_page) {
    ...
    RETURN_NOT_OK(read_file.ReadAsync(
        kPageSize * (read_page - file_start_page), Page(read_page), kPageSize, callback, context));
  }
  return Status::Ok;
}

template <class D>
Status PersistentMemoryMalloc<D>::AsyncFlushPage(
    uint32_t page,
    RecoveryStatus& recovery_status,
    AsyncCallback caller_callback,
    IAsyncContext* caller_context) {
...
template <class D>
void PersistentMemoryMalloc<D>::RecoveryReset(
    Address begin_address_,
