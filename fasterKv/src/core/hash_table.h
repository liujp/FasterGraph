...
class InternalHashTable {
 public:
  ...
  inline void Initialize(uint64_t new_size, uint64_t alignment) {
    ...
    if (size_ != new_size) {
      ...
      buckets_ =
          reinterpret_cast<HashBucket*>(aligned_alloc(alignment, size_ * sizeof(HashBucket)));
    }
 ...
 private:
  ...
  class AsyncIoContext : public IAsyncContext {
   public:
    AsyncIoContext(InternalHashTable* table_) : table{table_} {}
    /// The deep-copy constructor
    AsyncIoContext(AsyncIoContext& other) : table{other.table} {}
...
Status InternalHashTable<D>::Checkpoint(disk_t& disk, file_t&& file, uint64_t& checkpoint_size) {
  ...
  for (uint32_t idx = 0; idx < Constants::kNumMergeChunks; ++idx) {
    ...
    RETURN_NOT_OK(file_.WriteAsync(
        &bucket(idx * chunk_size), idx * write_size, write_size, callback, context));
  }
...
Status InternalHashTable<D>::Recover(disk_t& disk, file_t&& file, uint64_t checkpoint_size) {
  ...
  for (uint32_t idx = 0; idx < Constants::kNumMergeChunks; ++idx) {
    AsyncIoContext context{this};
    RETURN_NOT_OK(
        file_.ReadAsync(idx * read_size, &bucket(idx * chunk_size), read_size, callback, context));
  }
  return Status::Ok;
}
