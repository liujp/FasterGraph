...
class alignas(Constants::kCacheLineBytes) ThreadContext {
 public:
  ThreadContext() : contexts_{}, cur_{0} {}
...
class FasterKv {
 public:
  ...
  FasterKv(
      ...
      : min_table_size_{table_size},
        ...
        hlog{
            filename.empty() /*hasNoBackingStorage*/,
            log_size,
            epoch_,
            disk,
            disk.log(),
 ...
 public:
  ...
  bool Checkpoint(
      void (*index_persistence_callback)(Status result),
      void (*hybrid_log_persistence_callback)(Status result, uint64_t persistent_serial_num),
      Guid& token);
  bool CheckpointIndex(void (*index_persistence_callback)(Status result), Guid& token);
  bool CheckpointHybridLog(
      void (*hybrid_log_persistence_callback)(Status result, uint64_t persistent_serial_num),
      Guid& token);
  Status Recover(
      const Guid& index_token,
      const Guid& hybrid_log_token,
      uint32_t& version,
 ...
 private:
  ...
  inline const AtomicHashBucketEntry* FindEntry(KeyHash hash, HashBucketEntry& expected_entry)
      const;
  ...
  template <class C>
  inline Address TraceBackForKeyMatchCtxt(const C& ctxt, Address from_address, Address min_offset)
      const;
  inline Address TraceBackForKeyMatch(const key_t& key, Address from_address, Address min_offset)
      const;
  ...
  inline AtomicHashBucketEntry* FindTentativeEntry(
      KeyHash hash,
      HashBucket* bucket,
      uint8_t version,
      HashBucketEntry& expected_entry);
  // Looks for an entry that has the same
  inline bool HasConflictingEntry(
      KeyHash hash,
  ...
  inline Status HandleOperationStatus(
      ExecutionContext& ctx,
      pending_context_t& pending_context,
      OperationStatus internal_status,
      bool& async);
  inline Status
  PivotAndRetry(ExecutionContext& ctx, pending_context_t& pending_context, bool& async);
  ...
  inline Status
  IssueAsyncIoRequest(ExecutionContext& ctx, pending_context_t& pending_context, bool& async);

  void AsyncGetFromDisk(
      Address address,
      uint32_t num_records,
      AsyncIOCallback callback,
      AsyncIOContext& context);
  static void
  AsyncGetFromDiskCallback(IAsyncContext* ctxt, Status result, size_t bytes_transferred);
  ...
  void
  AddHashEntry(HashBucket*& bucket, uint32_t& next_idx, uint8_t version, HashBucketEntry entry);
...
inline void FasterKv<K, V, D>::StopSession() {
  ...
  while (thread_ctx().phase != Phase::REST || !thread_ctx().pending_ios.empty() ||
...
template <class K, class V, class D>
inline const AtomicHashBucketEntry* FasterKv<K, V, D>::FindEntry(
    KeyHash hash,
...
template <class K, class V, class D>
inline AtomicHashBucketEntry* FasterKv<K, V, D>::FindTentativeEntry(
    KeyHash hash,
    HashBucket* bucket,
    uint8_t version,
    HashBucketEntry& expected_entry) {
...
template <class K, class V, class D>
bool FasterKv<K, V, D>::HasConflictingEntry(
    KeyHash hash,
    const HashBucket* bucket,
    uint8_t version,
    const AtomicHashBucketEntry* atomic_entry) const {
  ...
  while (true) {
    for (uint32_t entry_idx = 0; entry_idx < HashBucket::kNumEntries; ++entry_idx) {
      HashBucketEntry entry = bucket->entries[entry_idx].load();
      if (entry != HashBucketEntry::kInvalidEntry && entry.tag() == tag &&
          atomic_entry != &bucket->entries[entry_idx]) {
        // Found a conflict.
        return true;
      }
...
template <class K, class V, class D>
inline AtomicHashBucketEntry* FasterKv<K, V, D>::FindOrCreateEntry(
    KeyHash hash,
    HashBucketEntry& expected_entry) {
  // Truncate the hash to get a bucket page_index < state[version].size.
...
template <class RC>
inline Status
FasterKv<K, V, D>::Read(RC& context, AsyncCallback callback, uint64_t monotonic_serial_num) {
  typedef RC read_context_t;
  typedef PendingReadContext<RC> pending_read_context_t;
  static_assert(
      std::is_base_of<value_t, typename read_context_t::value_t>::value,
...
template <class UC>
inline Status
FasterKv<K, V, D>::Upsert(UC& context, AsyncCallback callback, uint64_t monotonic_serial_num) {
  typedef UC upsert_context_t;
  typedef PendingUpsertContext<UC> pending_upsert_context_t;
  static_assert(
      std::is_base_of<value_t, typename upsert_context_t::value_t>::value,
...
template <class MC>
inline Status
FasterKv<K, V, D>::Rmw(MC& context, AsyncCallback callback, uint64_t monotonic_serial_num) {
  typedef MC rmw_context_t;
  typedef PendingRmwContext<MC> pending_rmw_context_t;
  static_assert(
      std::is_base_of<value_t, typename rmw_context_t::value_t>::value,
...
template <class DC>
inline Status
FasterKv<K, V, D>::Delete(DC& context, AsyncCallback callback, uint64_t monotonic_serial_num) {
  typedef DC delete_context_t;
  typedef PendingDeleteContext<DC> pending_delete_context_t;
  static_assert(
      std::is_base_of<value_t, typename delete_context_t::value_t>::value,
...
inline void FasterKv<K, V, D>::CompleteIoPendingRequests(ExecutionContext& context) {
  ...
  while (context.io_responses.try_pop(ctxt)) {
    ...
    if (internal_status == OperationStatus::SUCCESS) {
      ...
      result = HandleOperationStatus(
          context, *pending_context.get(), internal_status, pending_context.async);
    }
  ...
  for (size_t idx = 0; idx < size; ++idx) {
    ...
    switch (pending_context->type) {
      ...
      case OperationType::Upsert:
        internal_status =
            InternalUpsert(*static_cast<async_pending_upsert_context_t*>(pending_context.get()));
        break;
      default:
        assert(false);
...
inline OperationStatus FasterKv<K, V, D>::InternalRead(C& pending_context) const {
  ...
  if (address >= head_address) {
    ...
    if (!pending_context.is_key_equal(record->key())) {
      address = TraceBackForKeyMatchCtxt(
          pending_context, record->header.previous_address(), head_address);
    }
  }
  ...
  switch (thread_ctx().phase) {
    case Phase::PREPARE:
      ...
      if (!lock_guard.try_lock_old()) {
        ...
        if (latest_record_version > thread_ctx().version) {
          // CPR shift detected: we are in the "PREPARE" phase, and a record has a version later
          // than what we've seen.
          pending_context.go_async(
              thread_ctx().phase, thread_ctx().version, address, expected_entry);
          return OperationStatus::CPR_SHIFT_DETECTED;
        }
      }
      break;
...
create_record:
  ...
  new (new_record) record_t{
      RecordInfo{static_cast<uint16_t>(version), true, false, false, expected_entry.address()}};
...
create_record:
  ...
  new (record) record_t{
      RecordInfo{
          static_cast<uint16_t>(thread_ctx().version), true, true, false, expected_entry.address()},
  };
  pending_context.write_deep_key_at(const_cast<key_t*>(&record->key()));
...
template <class K, class V, class D>
template <class C>
inline Address FasterKv<K, V, D>::TraceBackForKeyMatchCtxt(
    const C& ctxt,
    Address from_address,
...
template <class K, class V, class D>
inline Address FasterKv<K, V, D>::TraceBackForKeyMatch(
    const key_t& key,
    Address from_address,
    Address min_offset) const {
  while (from_address >= min_offset) {
...
template <class K, class V, class D>
inline Status FasterKv<K, V, D>::HandleOperationStatus(
    ExecutionContext& ctx,
    pending_context_t& pending_context,
    OperationStatus internal_status,
    bool& async) {
  ...
  switch (internal_status) {
    ...
    case OperationStatus::RECORD_ON_DISK:
      if (thread_ctx().phase == Phase::PREPARE) {
        assert(
            pending_context.type == OperationType::Read ||
...
template <class K, class V, class D>
inline Status FasterKv<K, V, D>::PivotAndRetry(
    ExecutionContext& ctx,
    pending_context_t& pending_context,
    bool& async) {
  // Some invariants
...
template <class K, class V, class D>
inline Status FasterKv<K, V, D>::RetryLater(
    ExecutionContext& ctx,
    pending_context_t& pending_context,
    bool& async) {
...
inline constexpr uint32_t FasterKv<K, V, D>::MinIoRequestSize() const {
  return static_cast<uint32_t>(
      sizeof(value_t) + pad_alignment(record_t::min_disk_key_size(), alignof(value_t)));
}

template <class K, class V, class D>
inline Status FasterKv<K, V, D>::IssueAsyncIoRequest(
    ExecutionContext& ctx,
    pending_context_t& pending_context,
    bool& async) {
  ...
  async = true;
  AsyncIOContext io_request{
      this, pending_context.address, &pending_context, &thread_ctx().io_responses, io_id};
  AsyncGetFromDisk(
      pending_context.address, MinIoRequestSize(), AsyncGetFromDiskCallback, io_request);
  return Status::Pending;
}
...
template <class K, class V, class D>
void FasterKv<K, V, D>::AsyncGetFromDisk(
    Address address,
    uint32_t num_records,
    AsyncIOCallback callback,
    AsyncIOContext& context) {
...
template <class K, class V, class D>
void FasterKv<K, V, D>::AsyncGetFromDiskCallback(
    IAsyncContext* ctxt,
    Status result,
  ...
  if (result == Status::Ok) {
    ...
    if (record->min_disk_key_size() > record_size) {
      ...
      faster->AsyncGetFromDisk(
          context->address, record->min_disk_key_size(), AsyncGetFromDiskCallback, *context.get());
      context.async = true;
    } else if (record->min_disk_value_size() > record_size) {
      // Haven't read the full record in yet; I/O is not complete!
      faster->AsyncGetFromDisk(
          context->address,
          record->min_disk_value_size(),
      ...
      // Haven't read the full record in yet; I/O is not complete!
      faster->AsyncGetFromDisk(
          context->address, record->disk_size(), AsyncGetFromDiskCallback, *context.get());
      context.async = true;
      ...
      if (context->address >= faster->hlog.begin_address.load()) {
        faster->AsyncGetFromDisk(
            context->address, faster->MinIoRequestSize(), AsyncGetFromDiskCallback, *context.get());
        context.async = true;
      } else {
...
template <class K, class V, class D>
OperationStatus FasterKv<K, V, D>::InternalContinuePendingRead(
    ExecutionContext& context,
    AsyncIOContext& io_context) {
  if (io_context.address >= hlog.begin_address.load()) {
    async_pending_read_context_t* pending_context =
...
template <class K, class V, class D>
OperationStatus FasterKv<K, V, D>::InternalContinuePendingRmw(
    ExecutionContext& context,
    AsyncIOContext& io_context) {
  async_pending_rmw_context_t* pending_context =
      static_cast<async_pending_rmw_context_t*>(io_context.caller_context);
  ...
  if (address >= head_address && address != pending_context->entry.address()) {
    ...
    if (!pending_context->is_key_equal(record->key())) {
      ...
      address =
          TraceBackForKeyMatchCtxt(*pending_context, record->header.previous_address(), min_offset);
    }
  }
  assert(address >= pending_context->entry.address()); // part of the same hash chain

  if (address > pending_context->entry.address()) {
    // This handles two mutually exclusive cases. In both cases InternalRmw will be called
    // immediately:
    ...
    //  2) We can't trace the current hash bucket entry back to the record we've read (i.e. address
    //  < head_address)
    //     This is because part of the hash chain now extends to disk, thus we cannot check it right
    //     away Calling InternalRmw will result in launching a new search in the hash chain, by
    //     reading the newly introduced entries in the chain, so we won't miss any potential entries
    //     with same key.
    //
    pending_context->continue_async(address, expected_entry);
    return OperationStatus::RETRY_NOW;
  ...
  if (io_context.address < hlog.begin_address.load()) {
    ...
    new (new_record) record_t{
        RecordInfo{
            static_cast<uint16_t>(context.version), true, false, false, expected_entry.address()},
    };
    pending_context->write_deep_key_at(const_cast<key_t*>(&new_record->key()));
    pending_context->RmwInitial(new_record);
  } else {
    ...
    const record_t* disk_record =
        reinterpret_cast<const record_t*>(io_context.record.GetValidPointer());
    bool is_tombstone = disk_record->header.tombstone;
    uint32_t record_size =
        record_t::size(pending_context->key_size(), pending_context->value_size(disk_record));
    new_address = BlockAllocate(record_size);
    new_record = reinterpret_cast<record_t*>(hlog.Get(new_address));
...
Status FasterKv<K, V, D>::WriteCprContext() {
  ...
  if (std::fwrite(
          static_cast<PersistentExecContext*>(&prev_thread_ctx()),
          sizeof(PersistentExecContext),
          1,
          file) != 1) {
    std::fclose(file);
    return Status::IOError;
...
Status FasterKv<K, V, D>::CheckpointFuzzyIndex() {
  ...
  file_t ht_file =
      disk.NewFile(disk.relative_index_checkpoint_path(checkpoint_.index_token) + "ht.dat");
  RETURN_NOT_OK(ht_file.Open(&disk.handler()));
  RETURN_NOT_OK(state_[hash_table_version].Checkpoint(
      disk, std::move(ht_file), checkpoint_.index_metadata.num_ht_bytes));
  // Checkpoint the hash table's overflow buckets.
  file_t ofb_file =
      disk.NewFile(disk.relative_index_checkpoint_path(checkpoint_.index_token) + "ofb.dat");
  RETURN_NOT_OK(ofb_file.Open(&disk.handler()));
  RETURN_NOT_OK(overflow_buckets_allocator_[hash_table_version].Checkpoint(
      disk, std::move(ofb_file), checkpoint_.index_metadata.num_ofb_bytes));
...
Status FasterKv<K, V, D>::RecoverFuzzyIndex() {
  ...
  file_t ht_file =
      disk.NewFile(disk.relative_index_checkpoint_path(checkpoint_.index_token) + "ht.dat");
  RETURN_NOT_OK(ht_file.Open(&disk.handler()));
  RETURN_NOT_OK(state_[hash_table_version].Recover(
      disk, std::move(ht_file), checkpoint_.index_metadata.num_ht_bytes));
  // Recover the hash table's overflow buckets.
  file_t ofb_file =
      disk.NewFile(disk.relative_index_checkpoint_path(checkpoint_.index_token) + "ofb.dat");
  RETURN_NOT_OK(ofb_file.Open(&disk.handler()));
  return overflow_buckets_allocator_[hash_table_version].Recover(
      disk,
      std::move(ofb_file),
...
Status FasterKv<K, V, D>::RecoverHybridLog() {
  ...
  for (uint32_t page = start_page; page < end_page; ++page) {
    ...
    RETURN_NOT_OK(RecoverFromPage(
        page == start_page ? from_address : Address{page, 0},
        page + 1 == end_page ? to_address : Address{page, Address::kMaxOffset}));
...
Status FasterKv<K, V, D>::RecoverHybridLogFromSnapshotFile() {
  ...
  auto callback = [](IAsyncContext* ctxt, Status result) {
    CallbackContext<Context> context{ctxt};
    result = context->hlog->AsyncReadPagesFromSnapshot(
        *context->file, context->file_start_page, context->page, 1, *context->recovery_status);
  };
  ...
  uint32_t capacity = hlog.buffer_size();
  RecoveryStatus recovery_status{start_page, end_page};
  checkpoint_.snapshot_file = disk.NewFile(
      disk.relative_cpr_checkpoint_path(checkpoint_.hybrid_log_token) + "snapshot.dat");
  RETURN_NOT_OK(checkpoint_.snapshot_file.Open(&disk.handler()));

  // Initially issue read request for all pages that can be held in memory
  uint32_t total_pages_to_read = end_page - start_page;
  uint32_t pages_to_read_first = std::min(capacity, total_pages_to_read);
  RETURN_NOT_OK(hlog.AsyncReadPagesFromSnapshot(
      checkpoint_.snapshot_file, start_page, start_page, pages_to_read_first, recovery_status));
  ...
  for (uint32_t page = start_page; page < end_page; ++page) {
    ...
    if (Address{page + 1, 0} > from_address) {
      // handle start and end at non-page boundaries
      RETURN_NOT_OK(RecoverFromPage(
          page == from_address.page() ? from_address : Address{page, 0},
          page + 1 == end_page ? to_address : Address{page, Address::kMaxOffset}));
    }
    ...
    if (page + capacity < end_page) {
      Context context{
          hlog, checkpoint_.snapshot_file, start_page, page + capacity, recovery_status};
      RETURN_NOT_OK(hlog.AsyncFlushPage(page, recovery_status, callback, &context));
    } else {
      RETURN_NOT_OK(hlog.AsyncFlushPage(page, recovery_status, nullptr, nullptr));
    }
...
Status FasterKv<K, V, D>::RestoreHybridLog() {
  ...
  Address head_address =
      start_page == 0 ? Address{0, Constants::kCacheLineBytes} : Address{start_page, 0};
...
template <class K, class V, class D>
void FasterKv<K, V, D>::AddHashEntry(
    HashBucket*& bucket,
    uint32_t& next_idx,
    uint8_t version,
    HashBucketEntry entry) {
...
template <class K, class V, class D>
Address FasterKv<K, V, D>::TraceBackForOtherChainStart(
    uint64_t old_size,
    uint64_t new_size,
    Address from_address,
    Address min_address,
...
void FasterKv<K, V, D>::SplitHashTableBuckets() {
  ...
  for (uint64_t chunk = grow_.next_chunk++; chunk < grow_.num_chunks; chunk = grow_.next_chunk++) {
    ...
    for (uint64_t idx = 0; idx < upper_bound; ++idx) {
      ...
      HashBucket* old_bucket =
          &state_[grow_.old_version].bucket(chunk * kGrowHashTableChunkSize + idx);
      HashBucket* new_bucket0 =
          &state_[grow_.new_version].bucket(chunk * kGrowHashTableChunkSize + idx);
      HashBucket* new_bucket1 =
          &state_[grow_.new_version].bucket(old_size + chunk * kGrowHashTableChunkSize + idx);
      ...
      while (true) {
        for (uint32_t old_entry_idx = 0; old_entry_idx < HashBucket::kNumEntries; ++old_entry_idx) {
          ...
          const record_t* record = reinterpret_cast<const record_t*>(hlog.Get(old_entry.address()));
          KeyHash hash = record->key().GetHash();
          if (hash.idx(new_size) < old_size) {
            // Record's key hashes to the 0 side of the new hash table.
            AddHashEntry(new_bucket0, new_entry_idx0, grow_.new_version, old_entry);
            Address other_address = TraceBackForOtherChainStart(
                old_size, new_size, record->header.previous_address(), head_address, 0);
            ...
            AddHashEntry(new_bucket1, new_entry_idx1, grow_.new_version, old_entry);
            Address other_address = TraceBackForOtherChainStart(
                old_size, new_size, record->header.previous_address(), head_address, 1);
            if (other_address >= begin_address) {
  ...
  if (epoch_.FinishThreadPhase(Phase::GROW_IN_PROGRESS)) {
    ...
    GlobalMoveToNextState(
        SystemState{Action::GrowIndex, Phase::GROW_IN_PROGRESS, thread_ctx().version});
  } else {
      ...
      switch (next_state.phase) {
        ...
        case Phase::WAIT_FLUSH:
          ...
          if (fold_over_snapshot) {
            ...
            hlog.AsyncFlushPagesToFile(
                checkpoint_.log_metadata.flushed_address.page(),
    ...
    case Action::GrowIndex:
      switch (next_state.phase) {
        ...
        case Phase::GROW_IN_PROGRESS:
          // Swap hash table versions so that all threads will use the new version after populating
          // it.
...
void FasterKv<K, V, D>::HandleSpecialPhases() {
  ...
  do {
    ...
    SystemState current_state =
        (previous_state == final_state) ? final_state : previous_state.GetNextState();
        ...
        switch (current_state.phase) {
          ...
          case Phase::WAIT_FLUSH:
            ...
            if (!epoch_.HasThreadFinishedPhase(Phase::WAIT_FLUSH)) {
              ...
              if (fold_over_snapshot) {
                flushed =
                    hlog.flushed_until_address.load() >= checkpoint_.log_metadata.final_address;
              } else {
          ...
          case Phase::PERSISTENCE_CALLBACK:
            ...
            if (previous_state.phase == Phase::WAIT_FLUSH) {
              ...
              if (checkpoint_.hybrid_log_persistence_callback) {
                checkpoint_.hybrid_log_persistence_callback(
                    Status::Ok, prev_thread_ctx().serial_num);
              }
...
template <class K, class V, class D>
bool FasterKv<K, V, D>::Checkpoint(
    void (*index_persistence_callback)(Status result),
    void (*hybrid_log_persistence_callback)(Status result, uint64_t persistent_serial_num),
    Guid& token) {
...
template <class K, class V, class D>
bool FasterKv<K, V, D>::CheckpointIndex(
    void (*index_persistence_callback)(Status result),
    Guid& token) {
  // Only one thread can initiate a checkpoint at a time.
  SystemState expected{Action::None, Phase::REST, system_state_.load().version};
...
template <class K, class V, class D>
bool FasterKv<K, V, D>::CheckpointHybridLog(
    void (*hybrid_log_persistence_callback)(Status result, uint64_t persistent_serial_num),
    Guid& token) {
  // Only one thread can initiate a checkpoint at a time.
  SystemState expected{Action::None, Phase::REST, system_state_.load().version};
  ...
  if (!fold_over_snapshot) {
    ...
    checkpoint_.InitializeHybridLogCheckpoint(
        token, desired.version, false, Address::kInvalidAddress, hybrid_log_persistence_callback);
  }
  InitializeCheckpointLocks();
  // Let other threads know that the checkpoint has started.
  system_state_.store(desired.GetNextState());
...
Status FasterKv<K, V, D>::Recover(
  ...
  if (!system_state_.compare_exchange_strong(
          expected, SystemState{Action::Recover, Phase::REST, expected.version})) {
...
#define BREAK_NOT_OK(s)     \
  ...
  do {
    ...
    system_state_.store(
        SystemState{Action::Recover, Phase::REST, checkpoint_.log_metadata.version + 1});
...
template <class K, class V, class D>
bool FasterKv<K, V, D>::ShiftBeginAddress(
    Address address,
    GcState::truncate_callback_t truncate_callback,
    GcState::complete_callback_t complete_callback) {
  SystemState expected = SystemState{Action::None, Phase::REST, system_state_.load().version};
  if (!system_state_.compare_exchange_strong(
          expected, SystemState{Action::GC, Phase::REST, expected.version})) {
  ...
  epoch_.ResetPhaseFinished();
  uint64_t num_chunks =
      std::max(state_[resize_info_.version].size() / kGcHashTableChunkSize, (uint64_t)1);
...
bool FasterKv<K, V, D>::GrowIndex(GrowState::callback_t caller_callback) {
  SystemState expected = SystemState{Action::None, Phase::REST, system_state_.load().version};
  if (!system_state_.compare_exchange_strong(
          expected, SystemState{Action::GrowIndex, Phase::REST, expected.version})) {
  ...
  uint64_t num_chunks =
      std::max(state_[current_version].size() / kGrowHashTableChunkSize, (uint64_t)1);
...
bool FasterKv<K, V, D>::Compact(uint64_t untilAddress) {
  ...
  // instance of FASTER.
  int numOps = 0;
  ScanIterator<faster_t> iter(&hlog, Buffering::DOUBLE_PAGE, begin, Address(untilAddress), &disk);
  while (true) {
    auto r = iter.GetNext();
    if (r == nullptr)
      break;
  ...
  numOps = 0;
  ScanIterator<faster_t> iter2(
      &tempKv.hlog,
      Buffering::DOUBLE_PAGE,
      tempKv.hlog.begin_address.load(),
      tempKv.hlog.GetTailAddress(),
      &tempKv.disk);
  while (true) {
...
Address FasterKv<K, V, D>::LogScanForValidity(Address from, faster_t* temp) {
  ...
  while (from < sRO) {
    int numOps = 0;
    ScanIterator<faster_t> iter(&hlog, Buffering::DOUBLE_PAGE, from, sRO, &disk);
    while (true) {
      auto r = iter.GetNext();
      if (r == nullptr)
        break;
...
bool FasterKv<K, V, D>::ContainsKeyInMemory(key_t key, Address offset) {
  ...
  if (address >= offset) {
    ...
    const record_t* record = reinterpret_cast<const record_t*>(hlog.Get(address));
    if (key != record->key()) {
      address = TraceBackForKeyMatch(key, record->header.previous_address(), offset);
    }
  }
