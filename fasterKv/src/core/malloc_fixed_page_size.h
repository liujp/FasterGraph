...
class MallocFixedPageSize {
 ...
 private:
  ...
  class AsyncIoContext : public IAsyncContext {
   public:
    AsyncIoContext(alloc_t* allocator_) : allocator{allocator_} {}

    /// The deep-copy constructor
    AsyncIoContext(AsyncIoContext& other) : allocator{other.allocator} {}
...
Status MallocFixedPageSize<T, F>::Checkpoint(disk_t& disk, file_t&& file, uint64_t& size) {
  ...
  for (uint64_t idx = 0; idx < num_levels; ++idx) {
    ...
    RETURN_NOT_OK(
        file_.WriteAsync(page_array->Get(idx), idx * kWriteSize, kWriteSize, callback, context));
  }
...
Status MallocFixedPageSize<T, F>::Recover(
  ...
  for (uint64_t idx = 0; idx < num_file_levels; ++idx) {
    ...
    AsyncIoContext context{this};
    RETURN_NOT_OK(
        file_.ReadAsync(idx * kReadSize, page_array->GetOrAdd(idx), kReadSize, callback, context));
  }
  return Status::Ok;
}
...
FixedPageArray<T>* MallocFixedPageSize<T, F>::ExpandArray(array_t* expected, uint64_t new_size) {
  class Delete_Context : public IAsyncContext {
   public:
    Delete_Context(array_t* arr_) : arr{arr_} {}
    /// The deep-copy constructor.
    Delete_Context(const Delete_Context& other) : arr{other.arr} {}
